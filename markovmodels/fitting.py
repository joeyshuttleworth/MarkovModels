    ...
    for protocol in fitting_df[protocol_label].unique():
        for well in fitting_df['well'].unique():
            for sweep in fitting_df['sweep'].unique():
                ...
                # Choose finite parameters if possible
                if param_labels:
...
def make_prediction(model_class, args, well, sim_protocol, predict_sweep,
    ...
    if fitting_case in ['I', 'II']:
        ...
        if not np.all(np.isfinite(artefact_params)):
            logging.warning(f"{fitting_case} {model_class} {well} {protocol_fitted} {fit_sweep} Got non-finite artefact parameters: {artefact_params}")
            artefact_params = np.full(no_artefact_parameters, np.nan)
