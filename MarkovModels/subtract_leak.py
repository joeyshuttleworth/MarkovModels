    ...
    for well in args.wells:
        for protocol in args.protocols:
            ...
            if args.extra_points[1] > 0:
                extra_steps = np.array(list(itertools.islice(filter(lambda x : x[1] > 20, enumerate(protocol_voltages)), extra_points[1])))[extra_points[0]:,0].astype(int)
            else:
                extra_steps = []
            ...
            g_leak_before, E_leak_before, _, _, _, _, _ = fit_leak_lr(protocol_voltages, before_trace, dt=5e-4,  percentage_to_remove=args.percentage_to_remove, extra_points=extra_steps)
            g_leak_after, E_leak_after, _, _, _, _, _ = fit_leak_lr(protocol_voltages, after_trace, dt=5e-4,  percentage_to_remove=args.percentage_to_remove, extra_points=extra_steps)
