...
def generate_data(protocol, no_repeats):
    ...
    voltage_fig = plt.figure(figsize=args.figsize)
    current_fig = plt.figure(figsize=args.figsize)

    voltage_ax = voltage_fig.subplots()
    current_ax = current_fig.subplots()

    axs = [voltage_ax, current_ax]
    ...
    for repeat in range(no_repeats):
        ...
        if args.plot:
            ...
            axs[1].plot(times, data, color='grey', alpha=0.5)
            axs[0].set_xlabel('time / ms')
            axs[1].set_xlabel('time / ms')
            axs[0].set_ylabel('voltage / mV')
            axs[1].set_ylabel('current / nA')
            axs[0].plot(times, np.array([model.voltage(t) for t in times], label='voltage / mV'))
