...
def do_gpce(forward_problem, starting_parameters, first_V, second_V, distribution, quad_order=20, poly_order=30, plot=True):
    ...
    print(foo_approx)
    if plot:
        xs = np.linspace(distribution.mom(1)-math.sqrt(distribution.mom(2))*3,distribution.mom(1) + math.sqrt(distribution.mom(2))*3, 50)
        # plt.plot(xs, distribution.pdf(xs))
        plt.xlabel("p8")
        print(xs)
        plt.axvline(distribution.mom(1), linestyle="--", label="mean parameter value")
        plt.plot(xs, [foo_approx(x)[0].sum() for x in xs], label="gpce approximation" ,color="blue")
        plt.plot(xs, [forward_problem(x)[0] for x in xs], label="true value", color="red")
        plt.ylabel("current /nA")
        plt.legend()
        plt.savefig("normal_pdf_{}mV_{}mV.pdf".format(first_V, second_V))
        plt.clf()

        plt.fill_between(coordinates, expected-std, expected+std, alpha=0.3)
        plt.plot(coordinates, expected)
        plt.savefig("pseudo_spectral_plot_{}mV_{}mV.pdf".format(first_V, second_V))
        plt.xlabel("time /ms")
        plt.ylabel("current /nA")
        plt.clf()
        print("plotted")
...
if __name__ == "__main__":
    ...
    distribution = chaospy.Normal(starting_parameters[index_of_parameter], starting_parameters[index_of_parameter]/2)
